...
- Memory editor uses the same virtual row renderer with a navigable 1 MB window. Cursor navigation, page up/down, and address jumps now scroll smoothly with viewport-aware "ensure visible" behavior instead of relayouting a fixed 24-row window on every keystroke.
